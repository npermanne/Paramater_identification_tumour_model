...
class Grid:
    ...
    def __init__(self, xsize, ysize, sources,
        ...
        self.starting_glucose = 100.0
        self.starting_oxygen = 1000.0

        self.glucose = np.full((xsize, ysize), self.starting_glucose)
        self.oxygen = np.full((xsize, ysize), self.starting_oxygen)
    ...
    def neighbors(self, x, y):
        ...
        neigh = []
        for (i, j) in [(x, y), (x - 1, y - 1), (x - 1, y), (x - 1, y + 1), (x, y - 1), (x, y + 1), (x + 1, y - 1),
                       (x + 1, y),
                       (x + 1, y + 1)]:
